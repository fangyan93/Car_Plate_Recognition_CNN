...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("classifications_new.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("flattened_images_new.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
        print("error, unable to open flattened_images.txt, exiting program\n")               # show error message
        os.system("pause")
        return False                                                                        # and return False
    # end try
    print(len(npaFlattenedImages), len(npaFlattenedImages[0]))
    print(len(npaClassifications))
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        for i in range(0, len(listOfListsOfMatchingCharsInPlate)):
            ...
            # else:
            #     if (intLenOfLongestListOfChars == len(listOfListsOfMatchingCharsInPlate[i]) and 
            #         listOfListsOfMatchingCharsInPlate[intIndexOfLongestListOfChars][0].intBoundingRectArea < listOfListsOfMatchingCharsInPlate[i].intBoundingRectArea):
            #         intIndexOfLongestListOfChars = i
